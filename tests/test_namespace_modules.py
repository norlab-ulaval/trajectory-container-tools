...
class TestBackwardCompatibility:
    ...
    def test_original_rosbag_functions_exist(self):
        ...
        for func_name in original_functions:
            assert hasattr(
                tct, func_name
            ), f"Missing backward compatibility function: {func_name}"
...
class TestNamespacesConsistency:
    ...
    def test_all_namespaces_have_all_attribute(self):
        ...
        for namespace_name in namespaces:
            ...
            assert hasattr(
                namespace, "__all__"
            ), f"Missing __all__ in {namespace_name} namespace"
            assert isinstance(
                namespace.__all__, list
            ), f"__all__ is not a list in {namespace_name} namespace"
            assert (
                len(namespace.__all__) > 0
            ), f"Empty __all__ in {namespace_name} namespace"

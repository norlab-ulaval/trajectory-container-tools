...
class TestTrajectoryDataclassFromDataframeCase:
    ...
    def test_StatePose2D_init(self, mock_DF_2_trj_DC):
        md = mock_DF_2_trj_DC
        dc_ = StatePose2D(feature_name=md.name, x=md.a, y=md.b, yaw=md.c, timesteps_indices=md.ts, batch=True)
        print(dc_)

    def test_CmdStandard_init(self, mock_DF_2_trj_DC):
        md = mock_DF_2_trj_DC
        dc_ = CmdStandard(
                feature_name=md.name, linear_vel=md.a, angular_vel=md.b, timesteps_indices=md.ts, batch=True
                )
        print(dc_)
